...
6. **状态标志位审查**（如果引入了新的 bool/flag/状态变量）：
   - 列出所有**写入点**（哪些代码路径会 set/reset 该标志）
   - 列出所有**读取点**（哪些代码路径会检查该标志）
   - 验证路径组合是否存在意外状态（循环、死锁、竞争）
   - 画出状态转换图（如果涉及多个标志位交互）
...
- **不要自行分配编号** — 所有编号（§N / Phase N / SA-N 等）由主 session 在规划阶段预分配并写入占位 commit。不同仓库有独立的编号体系（如 nanobot-core 用 §N，web-chat 用 Phase N），SA 只使用本 prompt 中指定的编号。验收阶段新增的需求编号由主 session 分配，在已占位编号之后顺延
...
- **外部 API 验证**：涉及外部 API 调用的功能，**必须用真实 API 做至少一次端到端验证**，不能仅基于文档推测参数格式。验证失败时应在最终报告中明确标注（包括请求/响应详情），而非静默跳过
- （其他项目特定注意事项）
...
- [ ] **前端表现**（如涉及）：
  ...
  - [ ] 流式 vs 完成态一致
  - [ ] 刷新/切换 session 后状态恢复
- [ ] **dev 环境部署验证**（运行时行为类改动**必须**执行）：
  - 涉及状态管理、并发、消息处理等运行时行为的改动，**静态审查 + 单测 ≠ 验收通过**，必须在 dev 环境部署后进行实际交互验证
  - 部署到 dev 后，按用户验收指南中的操作步骤实际执行，观察真实行为
  - 可临时降低阈值（如 memory_window、超时时间）来加速触发目标场景
  - 验证通过后恢复原始阈值并重新部署确认
```
