...
4. **补建 todo**：复盘完成后，**agent 主动发起**（不等用户要求），梳理整个 batch 过程中（含验收、部署、调试）发现的所有新问题。流程：
   1. **梳理清单**：逐项列出问题，与现有 todo 对比，过滤已存在的
   2. **分类标注**：对每项标注建议的处理方式——
      - 代码 bug → 建 todo（priority=high，tag=bug）
      - 功能需求 / 技术债 → 建 todo（priority=medium，tag=feature）
      - 已决定暂缓的问题 → 记录到 MEMORY Known Bugs，注明暂缓原因，不建 todo
      - 已通过流程改进解决的 → 不建 todo（已落地到文档）
   3. **用户确认**：将完整清单（含分类和建议处理方式）呈现给用户，**等用户确认后再实际创建**
   4. **关联原始信息**：创建 todo 时，notes 中必须包含：
      - 来源 batch 名称（如 batch-20260320）
      - 发现场景（验收/部署/调试/复盘）
      - 相关 session key（如有）
      - 问题的具体描述和复现方式（如适用）
