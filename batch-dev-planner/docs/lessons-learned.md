...
## 14. Batch-20260320 经验教训

### 14.1 验收策略

- **Dev 环境部署验证是唯一有效的运行时 Bug 发现手段** — batch-20260320 Plan Core 验收中，静态审查 + 单测发现率 0%，所有 3 个 Bug（§64 无限循环、§65 tool-aligned cut、warning 竞争）均在 dev 部署后发现 (batch-20260320 #1)
- **临时降低阈值是加速验证的有效技巧** — 如将 memory_window 从 100 降到 20，快速触发 consolidation/warning 场景 (batch-20260320 #2)
- **用户干预是当前验收质量的核心保障** — batch-20260320 Plan Core 验收中用户 6 次关键干预（要求 dev 部署、指定验证场景、要求恢复配置等），均为 Agent 未主动执行的步骤 (batch-20260320 #3)
- **跨 session 验收需要 review-state 实时同步** — 验收跨越多个 session 时，review-state 文档是上下文同步的唯一可靠来源，不能依赖 session 历史 (batch-20260320 #4)

### 14.2 外部 API 集成

- **SA 外部 API 集成必须实测，不能基于文档推测** — batch-20260320 Plan ASR 中，飞书 ASR API 的 file_id 长度限制（恰好 16 位）、engine_type 限制（仅 16k_auto）等均无法从文档推断，必须实际调用验证 (batch-20260320 #5)

### 14.3 编号管理

- **编号应在开发前通过占位 commit 预分配** — batch-20260320 中 Phase 61 被"轮询修复"和"integration tests"重复使用，事后无法修改 git history，只能在 DEVLOG 中修正。正确做法是规划后先做编号占位 commit (batch-20260320 #6)
- **不同仓库编号体系独立** — nanobot-core 用 §N，web-chat 用 Phase N，各自维护最大编号，SA 不得跨仓库分配或自行编号 (batch-20260320 #7)

### 14.4 分支策略

- **独立 skill 仓库也必须走 feature branch** — batch-20260320 中 feishu-parser 直接在 main 开发违反了分支策略。即使是独立仓库，也应拉 feature branch，验收通过后 merge --no-ff (batch-20260320 #8)

### 14.5 状态标志位

- **引入新状态标志位时必须全面审查读写点** — batch-20260320 §64 的 `_warned_this_turn` 标志在 cleanup 路径被意外重置导致无限循环。设计审查应列出所有写入点、读取点及路径组合 (batch-20260320 #9)
