...
**3. 分析 handle-completion 结果**：
   - `dispatch_role` + `spawn_instruction` → spawn 新 Worker（传入 `max_iterations`）
   - `mark_done` / `promote_to_review` → ✅ 正常流转
   - `mark_blocked` → **不要无脑接受！** 分析原因：
     - "no worker report found" → `follow_up` 让 Worker 补写报告后重试
     - "partial" → 分析是"没做完"还是"真阻塞"，没做完就 `follow_up` 继续
     - 真正的外部阻塞 → 接受 blocked，通知用户
...
调度器输出 JSON，包含 `spawn_instructions` 数组。每个元素包含：
- `task_id`: 任务 ID
- `task_prompt`: 发给 worker 的完整任务指令
- `title`: 显示名称
- `priority`: 优先级
- `template`: 工作组模板
- `max_iterations`: Worker 最大迭代数（developer=60, tester=30）

如果 `spawn_instructions` 为空，说明没有需要派发的任务，直接跳到 Step 4。

### Step 3: 用 spawn tool 派发 worker subagent

对每个 spawn instruction，调用 spawn tool:

```
spawn(task="<task_prompt 的完整内容>", max_iterations=<spawn instruction 中的 max_iterations 值>)
```

⚠️ 注意：
- task_prompt 内容可能很长（包含完整任务指令），必须完整传入 spawn 的 task 参数
- **必须传入 max_iterations 参数**（developer=60, tester=30），该值在 spawn instruction 的 `max_iterations` 字段中
- spawn 是异步的 — 不需要等待 worker 完成
- worker subagent 完成后，框架会自动向本 session 发送 `[Subagent Result Notification]` 消息
- 可以同时 spawn 多个 worker（每个 spawn instruction 一个）
...
- `action: dispatch_role` + `spawn_instruction` → 用 spawn 派发新 Worker（传入 `max_iterations`）
- `action: mark_done` → 任务完成 ✅
- `action: promote_to_review` → 任务进入 review 流程
- `action: mark_blocked` → 按**情况 C/D** 的逻辑处理，不要无脑接受
