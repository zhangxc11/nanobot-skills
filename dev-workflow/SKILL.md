...
2. **方案文档** — 写入 ARCHITECTURE.md，必须包含验收 Checklist（可测试的通过条件），每项标注「可自验 ✅」或「需人工 👤」
   - **设计哲学**：如项目有长期演进需求，建议在 ARCHITECTURE.md 第一章写「设计哲学」——顶层设计理念和核心约束，后续迭代时各角色据此校验方案一致性
   - 已有设计哲学章节的项目：先通读，确保新方案不违反已有原则；如需修改原则，必须显式说明理由
...
### Checkpoint
- [ ] Glossary / 关键术语定义了？
- [ ] ARCHITECTURE.md 有设计哲学章节？→ 新方案与之无冲突？（修改原则需显式说明理由）
...
- ARCHITECTURE.md 如有设计哲学章节，新方案必须符合；修改原则需显式说明理由
...
3. 读 `docs/ARCHITECTURE.md` — 理解技术设计；如有设计哲学章节，重点通读作为后续校验基准
