...
## T-20260402-002: Cross Check 整改 Phase 1 — Git Hook / 二次确认 / 通知验证

**Date**: 2026-04-02 | **Status**: ✅ Complete
**Design**: [cross-check-remediation-designer.md](../../data/brain/designs/cross-check-remediation-designer.md)

### 目标
Cross Check 系统整改 Phase 1 三项最高风险整改：
1. Git commit-msg hook — 强制 commit message 包含 Task ID
2. 飞书回复不可逆操作二次确认 — cancel/reject 必须经用户确认
3. validate_notification() — 通知发送前状态一致性校验

### 修改清单

#### P1-1: Git commit-msg hook（环节 13 代码上线）
- `.git/hooks/commit-msg`（nanobot + web-chat 两个仓库均已安装）
- 新增可分发资产:
  - `scripts/git_hooks/commit-msg` — hook 源文件
  - `scripts/install_git_hooks.sh` — 安装脚本（支持 nanobot/web-chat 两仓库）
- 校验规则: commit message 必须包含 `T-YYYYMMDD-NNN` 格式 Task ID
- 豁免: Merge/Revert/Initial commit 自动跳过
- 绕过: `SKIP_TASK_ID_CHECK=1` 环境变量或 `--no-verify`

#### P1-2: 飞书回复不可逆操作二次确认（环节 11 飞书回复路由）
- `trigger_scheduler.py` L65: 新增 `IRREVERSIBLE_CONFIRM_ENABLED` feature flag
- `trigger_scheduler.py` L67-87: 新增 `IRREVERSIBLE_CONFIRM_RULES` 常量（完整二次确认规则文本）
- `trigger_scheduler.py` L340: 调度器 prompt 中注入二次确认提醒
- `trigger_scheduler.py` L422: 调度器 prompt 末尾注入完整确认规则
- 规则: cancel/reject 操作先回复确认提示，用户回复"确认取消/拒绝 xxx"后才执行

#### P1-3: validate_notification() 通知验证（环节 10 飞书通知内容）
- `scheduler.py` L364: 新增 `CROSS_CHECK_ENABLED` feature flag
- `scheduler.py` L370-371: 新增 `NOTIFICATION_VALIDATE_ENABLED` / `IRREVERSIBLE_CONFIRM_ENABLED` flags
- `scheduler.py` L1126-1183: 新增 `validate_notification()` 函数
  - Check 1: 通知文本包含与 new_state 匹配的关键词
  - Check 2: 通知文本包含 task_id（full 或 short 形式）
  - Check 3: 通知文本包含任务标题（至少部分匹配）
  - Check 4: review 通知必须包含 Go/NoGo 操作指引
- `scheduler.py` L1240-1249: `notify_task_state_change()` 发送前调用验证，失败记录 decisions.jsonl 但不阻塞发送

### Feature Flags
| Flag | 环境变量 | 默认值 | 控制范围 |
|------|---------|--------|---------|
| `CROSS_CHECK_ENABLED` | `CROSS_CHECK_ENABLED` | `"1"` (开启) | 全局 cross-check 总开关 |
| `NOTIFICATION_VALIDATE_ENABLED` | `NOTIFICATION_VALIDATE_ENABLED` | `"1"` (开启) | `validate_notification()` |
| `IRREVERSIBLE_CONFIRM_ENABLED` | `IRREVERSIBLE_CONFIRM_ENABLED` | `"1"` (开启) | 二次确认规则注入 |
