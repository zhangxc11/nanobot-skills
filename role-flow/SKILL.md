...
调度方 spawn 每个角色 subagent 时，**必须使用以下模板**构造 prompt。模板确保角色读取自身 ROLE.md 和 pattern 文件，从源头获取行为规范和流程定义。

```
## 角色：{角色名}（{阶段说明}）

**⚠️ 必须先读取以下文件，理解你的角色定义和工作规范：**
1. `{ROLE_FLOW_DIR}/roles/{role-name}/ROLE.md`
2. `{ROLE_FLOW_DIR}/patterns/{pattern-name}.md`
...
### 报告格式
完成后按 ROLE.md 中定义的输出格式输出报告。
```

<!-- ⚠️ 不要在 spawn prompt 中添加输出格式定义或流程定义。角色从 ROLE.md 和 pattern 文件获取完整信息。 -->

**模板设计要点**：

1. **ROLE.md 和 pattern 文件读取指令放在 prompt 最前面**，利用 primacy bias 确保被优先执行
2. **不在 spawn prompt 中提供流程定义** — 角色自己从 pattern 文件获取
3. **不在 spawn prompt 中提供输出格式** — 角色自己从 ROLE.md 获取
4. **模板是通用的** — 适用于所有角色（符合 P8 通用机制优先）
5. **Architect 多阶段**：spawn 时在标题中明确当前阶段，如 `Architect（设计阶段）`、`Architect（代码审查阶段）`、`Architect（测试审查阶段）`

### 调度方注意事项

- **spawn 角色时必须使用上方的 spawn 模板**，确保每个角色收到的 prompt 包含 ROLE.md 和 pattern 文件读取指令
- **禁止在 spawn prompt 中提供简化版流程定义或输出格式** — 这些信息由角色自己从 ROLE.md 和 pattern 文件获取
- **Architect 多阶段注意**：Architect 在 dev-pipeline 中承担设计、代码审查、测试审查三种职责，spawn 时必须在标题中明确当前阶段
